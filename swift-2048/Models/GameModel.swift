  ...
  func userHasWon() -> (Bool, (Int, Int)?) {
    for i in 0..<dimension {
      for j in 0..<dimension {
        ...
        if case let .tile(v) = gameboard[i, j], v >= threshold { // user win condition
          return (true, (i, j))
        }
      }
    }
      ...
      for object in orders {
        ...
        case let MoveOrder.singleMoveOrder(s, d, v, wasMerge):
          ...
          if wasMerge {
              ...
              case 233,144: // if gameboard has 233 or 144 as the highest tile ,the remaining points to be gained will be 2 times more than usual
                  score += 2*v
              case 377:  // if the gameboard has 377 as the highest tile, the remaining point to be gained  will be 4 times more than usual
                  score += 4*v
              case 610:  // if the gameboard has 610 as the highest tile, the remaining points to be gained will be 6 times more than usual
                  score += 6*v
              default:
                  score += v
              }
    ...
    for (idx, token) in group.enumerated() {
      ...
      case let .noAction(s, v)
        where (idx < group.count-1
               && isFibSum(number1:v,number2:group[idx+1].getValue()) 
      ...
      case let t where (idx < group.count-1 && isFibSum(number1:t.getValue(),number2:group[idx+1].getValue())): 
